#!/usr/bin/env python3
"""validate_observability_findings.py -- Capa 3 (Observabilidad) de KDD, capa OPCIONAL.

Valida un `findings.json` YA SELLADO (por el proceso descrito en la skill
kdd-observability-scan: identificar rutas criticas y revisar cobertura de
logging/alertas/tracing) contra la politica declarativa de
examples/rules/observability-findings.rules.json, via el mismo
...
si <scan_dir>/findings.json no existe, se reporta INFO y exit 0 -- un fork
sin capa de observabilidad configurada queda verde sin editar nada.

Uso:
    python scripts/validate_observability_findings.py [scan_dir]

[scan_dir] default: 'observability/scan'. exit 0 sin violaciones (o capa
...
_RULES_PATH = os.path.join(
    ...
    "..", "examples", "rules", "observability-findings.rules.json",
)
_DEFAULT_SCAN_DIR = "observability/scan"
...
def flatten(findings_doc: dict) -> dict:
    ...
    tipo de gap, severidad y longitud de la remediacion. No se reinterpreta
    el contenido -- valores ausentes se aplanan a 0/None y el rule engine
    los trata como violaciones de 'required'/'bounds' segun corresponda."""
    flat = []
...
def main(argv=None):
    ...
    if len(args) > 1:
        print("Uso: validate_observability_findings.py [scan_dir]", file=sys.stderr)
        return 2
    ...
    if findings_doc.get("documentType") != "kdd-observability.findings":
        print(
            ...
            "no parece un artefacto sellado del dominio de observabilidad",
            file=sys.stderr,
        )
        return 2
    ...
    if violations:
        print(f"FAIL: {len(violations)} violacion(es) de politica de observabilidad")
        for violation in violations:
            print(" -", violation)
        return 1

    print(
        f"OK: {findings_path} conforme a la politica de observabilidad "
