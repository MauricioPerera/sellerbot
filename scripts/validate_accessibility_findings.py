#!/usr/bin/env python3
"""validate_accessibility_findings.py -- Capa 3 (Accesibilidad) de KDD, capa OPCIONAL.

Valida un `findings.json` YA SELLADO (por el proceso descrito en la skill
kdd-accessibility-scan: correr un scanner automatizado tipo axe-core/
Lighthouse/pa11y y/o revision manual, normalizar al schema de este dominio)
contra la politica declarativa de
examples/rules/accessibility-findings.rules.json, via el mismo
...
Distinto de validate_ux_page.py (linter estatico sobre HTML propio del
repo, sin ejecutar nada): este gate audita hallazgos de una auditoria
externa sobre el producto en ejecucion. Ambos pueden coexistir sin
conflicto -- ver knowledge/data_models/accessibility_findings.md.

Capa opcional (mismo patron que validate_security_findings.py /
validate_compliance_findings.py / validate_privacy_findings.py): si
<scan_dir>/findings.json no existe, se reporta INFO y exit 0 -- un fork
sin capa de accesibilidad configurada queda verde sin editar nada.

Uso:
    python scripts/validate_accessibility_findings.py [scan_dir]

[scan_dir] default: 'accessibility/scan'. exit 0 sin violaciones (o capa
...
_RULES_PATH = os.path.join(
    ...
    "..", "examples", "rules", "accessibility-findings.rules.json",
)
_DEFAULT_SCAN_DIR = "accessibility/scan"
...
def flatten(findings_doc: dict) -> dict:
    ...
    criterio WCAG, impacto y longitud de la remediacion. No se
    ...
    for finding in findings_doc.get("findings", []) or []:
        ...
        flat.append({
            ...
            "wcagCriterion": finding.get("wcagCriterion"),
            "impactLevel": finding.get("impact"),
...
def main(argv=None):
    ...
    if len(args) > 1:
        print("Uso: validate_accessibility_findings.py [scan_dir]", file=sys.stderr)
        return 2
    ...
    if findings_doc.get("documentType") != "kdd-accessibility.findings":
        print(
            f"ERROR [DOCUMENT_TYPE] {findings_path}: documentType inesperado -- "
            "no parece un artefacto sellado del dominio de accesibilidad",
            file=sys.stderr,
        )
        return 2
    ...
    if violations:
        print(f"FAIL: {len(violations)} violacion(es) de politica de accesibilidad")
        for violation in violations:
            print(" -", violation)
        return 1

    print(
        f"OK: {findings_path} conforme a la politica de accesibilidad "
